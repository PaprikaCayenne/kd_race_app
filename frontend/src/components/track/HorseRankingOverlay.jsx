...
export default function HorseRankingOverlay({
  ...
  return (
    <div
      className="absolute bg-gradient-to-b from-[#eef6ff]/95 via-white/95 to-[#e2f2ff]/90 border border-sky-200 p-3 rounded-2xl shadow-2xl z-50 animate-fadeIn"
      style={panelStyle}
    >
      <div
      ...
      <ol className="space-y-1.5 text-sm">
        {ranking.map((h, i) => {
          ...
          return (
            <li
              key={h.id}
              className="flex items-center px-2 py-1.5 bg-white/90 rounded-xl shadow-sm border border-sky-100 gap-2"
            >
              <span
              ...
              <HorseSprite bodyHex={h.bodyHex} saddleHex={h.saddleHex} alt={h.name} className="w-12 h-12 shrink-0" />
